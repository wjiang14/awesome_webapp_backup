...
# test if __user__ of request is admin
...
def api_blogs(*, page='1'):
    ...
    return dict(dict(page=p, blogs=blogs))
...
# update blog, we need the blog id
@post('/api/blogs/{id}')
def api_update_blog(id, request, *, name, summary, content):
    check_admin(request)  # check if user is admin'
    blog = yield from Blog.find(id)
    if not name or not name.strip():
        raise APIValueError('name', 'name cannot be empty.')
...
def api_delete_blog(request, *, id):
    check_admin(request)
    blog = yield from Blog.find(id)
    yield from blog.remove()  # remove the blog
    return dict(id=id)
